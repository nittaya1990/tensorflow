...
// template <typename T>
// TODO(cais): Confirm save removal
class CopyOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    //     is_input_ref ? context->mutable_input(0, false) : context->input(0);
    // TODO(cais): Confirm save change.
    ...
    if (off_host_input) {
      ...
      // Tensor host_tensor(tensorflow::cpu_allocator(), src_tensor.dtype(),
      //                    src_tensor.shape());
      Notification done_copy;
      GPUUtil::CopyGPUTensorToSameGPU(device, device_ctxt, &src_tensor,
                                      copied_tensor,
                                      [&done_copy](const Status& s) {
                                        // TODO(cais): non-ok status.
                                        done_copy.Notify();
                                      });

...
"^tensorflow/python/platform/gfile\.py.*\[E0301.*non-iterator"
  ...
  if [[ ! -z "$2" ]] && [[ "$2" == "--incremental" ]]; then
    PYTHON_SRC_FILES=$(get_py_files_to_check)

    echo "do_pylint will perform checks on only the Python files changed in "\
"the last non-merge git commit due to the --incremental flag:"
...
do_pep8() {
  ...
  if [[ "$1" == "--incremental" ]]; then
    ...
    echo "do_pep8 will perform checks on only the Python files changed in "\
"the last non-merge git commit due to the --incremental flag:"
    echo "${PYTHON_SRC_FILES}"
    echo ""
  else

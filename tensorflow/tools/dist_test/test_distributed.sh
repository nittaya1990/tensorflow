...
if [[ ! -z "$TF_DIST_GRPC_SERVER_URL" ]]; then
  DOCKER_ENV="${DOCKER_ENV} -e TF_DIST_GRPC_SERVER_URL="\
"${TF_DIST_GRPC_SERVER_URL}"
fi
if [[ ! -z "$TF_DIST_GCLOUD_PROJECT" ]]; then
  DOCKER_ENV="${DOCKER_ENV} -e TF_DIST_GCLOUD_PROJECT=${TF_DIST_GCLOUD_PROJECT}"
fi
if [[ ! -z "$TF_DIST_GCLOUD_COMPUTE_ZONE" ]]; then
  DOCKER_ENV="${DOCKER_ENV} -e TF_DIST_GCLOUD_COMPUTE_ZONE="\
"${TF_DIST_GCLOUD_COMPUTE_ZONE}"
fi
if [[ ! -z "$TF_DIST_CONTAINER_CLUSTER" ]]; then
  DOCKER_ENV="${DOCKER_ENV} -e TF_DIST_CONTAINER_CLUSTER="\
"${TF_DIST_CONTAINER_CLUSTER}"
fi
